...
export class GameScreen {
    ...
    _bindEvents() {
        ...
        this.element.querySelector('#btnSubmitScore').addEventListener('click', () => {
            this.app.soundManager.stopGameOverSound();
            this.app.leaderboard.addEntry(this.app.localService.getPlayerName(), this.gameManager.score, this.gameManager.currentMap.displayName);
            this.app.localService.addCoins(this.gameManager.coins);
            if (this.gameManager.diamonds > 0) this.app.localService.setDiamonds(this.app.localService.getDiamonds() + this.gameManager.diamonds);
            this.backToMenu();
        });
    ...
    startGame() {
        const selMapName = this.app.localService.load('selectedMap', 'FOREST');
        const mapList = getMapList();
        let mapType = mapList.find(m => m.name === selMapName) || mapList[0];
        
        const skinColor = this.app.localService.getSelectedSkin();
        const skinShape = this.app.localService.getSelectedShape();
    ...
    backToMenu() {
        ...
        if (!this.gameManager.gameOver) {
            ...
            if (this.gameManager.diamonds > 0) {
                this.app.localService.setDiamonds(this.app.localService.getDiamonds() + this.gameManager.diamonds);
    ...
    loop(timestamp) {
        ...
        if (this.gameManager.gameOver) {
            ...
            this.app.localService.addStat('totalMatches', 1);
            this.app.localService.updateDailyQuestProgress('totalMatches', 1);
            
            this.app.localService.addStat('totalFood', this.gameManager.foodEaten);
            this.app.localService.updateDailyQuestProgress('totalFood', this.gameManager.foodEaten);
            
            this.app.localService.addStat('totalAbilities', this.gameManager.abilitiesUsed);
            this.app.localService.updateDailyQuestProgress('totalAbilities', this.gameManager.abilitiesUsed);
