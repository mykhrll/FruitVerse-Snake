...
export class ShopScreen {
    ...
    updateUI() {
        const coins = this.app.storage.getCoins();
        const diamonds = this.app.storage.getDiamonds();
        this.element.querySelector('#shopCoinsVal').innerText = coins;
        this.element.querySelector('#shopDiamondsVal').innerText = diamonds;
        
        const ownedSkins = this.app.storage.getOwnedSkins();
        const selectedSkin = this.app.storage.getSelectedSkin();
        
        const ownedShapes = this.app.storage.getOwnedShapes();
        const selectedShape = this.app.storage.getSelectedShape();
    ...
    handleAction(id, type) {
        ...
        let ownedList = type === 'color' ? this.app.storage.getOwnedSkins() : this.app.storage.getOwnedShapes();
        
        if (ownedList.includes(id)) {
            // Equip
            if (type === 'color') this.app.storage.setSelectedSkin(id);
            else this.app.storage.setSelectedShape(id);
        } else {
            // Buy
            let bal = type === 'color' ? this.app.storage.getCoins() : this.app.storage.getDiamonds();
            if (bal >= itemDef.price) {
                if (type === 'color') {
                    this.app.storage.setCoins(bal - itemDef.price);
                    ownedList.push(id);
                    this.app.storage.setOwnedSkins(ownedList);
                    this.app.storage.setSelectedSkin(id);
                } else {
                    this.app.storage.setDiamonds(bal - itemDef.price);
                    ownedList.push(id);
                    this.app.storage.setOwnedShapes(ownedList);
                    this.app.storage.setSelectedShape(id);
                }
            }
        }
